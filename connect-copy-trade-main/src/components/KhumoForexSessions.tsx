    ...
    try {
      ...
      try {
        ...
        for (const sym of relevantPairs.slice(0, 3)) {
          ...
          if (p) livePrices[sym] = p;
        }
        if (Object.keys(livePrices).length > 0) {
          livePriceContext = `\n\nCURRENT LIVE MARKET PRICES (use these EXACTLY for entry; SL/TP must be realistic offsets from these):\n${Object.entries(livePrices).map(([s, p]) => `- ${s}: ${p}`).join("\n")}`;
        }
...
Relevant currency pairs for active sessions: ${relevantPairs.join(", ")}${livePriceContext}

Provide ONE specific trading idea with:
1. Symbol (from the relevant pairs)
2. Direction (BUY or SELL)
3. Entry price (USE THE LIVE PRICE if provided above for that symbol)
4. Stop Loss level (realistic — e.g. 30 pips for forex, 50 points for XAUUSD, 0.5% for synthetics)
5. Take Profit level (1.5-2x the SL distance for good R:R)
